...
for(element in 1:length(Code_list)){
  ...
  Num_element <- paste('Number_of_' ,name)
  value <- sum(code_file$Codes == name, na.rm = TRUE)
  colnames(Tot_code)[ element] <- Num_element
  Tot_code[1, element] <- value
}

### from code_file check the code name and the food_id to find the element in Food_db, in the line of the corresponding code in Avg sum the value for CNP
for(line in 1:327){
...
for(column in 3:5){
  for( i in 1:length(Code_list)){
    Avg[i,column] <- Avg[i,column]/Tot_code[1, i]
  } 
}     
...
write.xlsx(Avg2, file= 'food_category_CNP_db.xlsx', sheetName = 'Food cat comp', col.names = TRUE, row.names = FALSE, append = TRUE )
